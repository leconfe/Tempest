...
<section class="py-6 sm:py-8 md:py-12 bg-white">
	<div class="container mx-auto px-4">
		<div class="max-w-7xl mx-auto">
			<h3
				class="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-6 sm:mb-8 md:mb-10 relative inline-block w-full text-gray-900">
				Committees
				<span
					class="garis absolute left-1/2 -bottom-3 sm:-bottom-4 transform -translate-x-1/2 w-16 sm:w-20 md:w-24 h-1 bg-purple-600"></span>
			</h3>
			...
			<div class="space-y-12 sm:space-y-16 md:space-y-20">
				@foreach ($committeeRoles as $role)
					@if ($role->committees->isNotEmpty())
						<div
							class="bg-white rounded-2xl sm:rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] p-6 sm:p-8 md:p-2 lg:p-16 transform hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)] transition-all duration-500">
							<h2
								class="text-xl sm:text-2xl md:text-3xl font-bold mb-8 sm:mb-12 md:mb-16 flex items-center justify-center text-gray-900 md:mt-8 lg:mt-2">
								<span class="relative">
									{{ $role->name }}
									<span class="garis absolute -bottom-1 sm:-bottom-2 left-0 w-full h-0.5"></span>
								</span>
								<svg class="speak-icon w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 ml-2 sm:ml-3"
							...
							<div class="items-center justify-center gap-6 sm:gap-8 md:gap-12 flex flex-wrap">
								@foreach ($role->committees as $committee)
									<div class="w-full max-w-xs flex">
										<div
											class="bg-gradient-to-b from-white to-gray-50/50 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 transition duration-500 transform hover:-translate-y-2 hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-gray-100 flex flex-col w-full">
											<div class="relative mb-4 sm:mb-5 md:mb-6 flex justify-center">
												<div
													class="w-28 h-28 sm:w-32 sm:h-32 md:w-40 md:h-40 flex-shrink-0">
											...
											<div
												class="text-center flex-grow flex flex-col justify-between min-h-[100px]">
												<div>
													<h4
														class="text-lg sm:text-xl font-bold mb-2 sm:mb-3 text-gray-900 line-clamp-2 min-h-[2.5em]">
														{{ $committee->fullName }}</h4>
													@if ($committee->getMeta('affiliation'))
														<p
															class="text-xs sm:text-sm text-gray-600 px-2 sm:px-3 md:px-4 leading-relaxed line-clamp-3">
															{{ $committee->getMeta('affiliation') }}</p>
													@endif
												</div>
												@if ($committee->getMeta('scopus_url') || $committee->getMeta('google_scholar_url') || $committee->getMeta('orcid_url'))
													<div
														class="flex justify-center items-center space-x-4 pt-4 border-t border-gray-100">
														@if ($committee->getMeta('orcid_url'))
															<a href="{{ $committee->getMeta('orcid_url') }}"
																target="_blank"
														...
														@if ($committee->getMeta('google_scholar_url'))
															<a href="{{ $committee->getMeta('google_scholar_url') }}"
																...
																<x-academicon-google-scholar
																	class="w-7 h-7 opacity-80 hover:opacity-100" />
															</a>
														@endif

														@if ($committee->getMeta('scopus_url'))
															<a href="{{ $committee->getMeta('scopus_url') }}"
																target="_blank"
